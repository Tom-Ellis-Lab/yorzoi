...
from yorzoi.eval.utils import evaluate_model, load_model_for_testing
from yorzoi.eval.promoter.utils import evaluate as eval_DREAM
from yorzoi.eval.terminator.utils import evaluate as eval_Shalem
from yorzoi.eval.scramble.utils import evaluate as eval_Brooks
...
def parse_eval_types(eval_str: str) -> List[str]:
    ...
    if invalid_types:
        raise typer.BadParameter(
            f"Invalid evaluation types: {invalid_types}. Valid types are: {VALID_EVALS}"
        )
    return eval_types
...
def evaluate(
    ...
    model, test_loader, track_order, model_resolution = load_model_for_testing(
        base_path=str(bp), device=device
    )
    print("\tDone.")
    ...
    if "default" in eval_types or "baseline" in eval_types:
        ...
        ), "test_loader should not be None for baseline evaluation"
