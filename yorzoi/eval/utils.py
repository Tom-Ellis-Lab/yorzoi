        ...
        track_mrae.append(
            {
                ...
                "track_name": track_name_order[track_idx],
                "mean_mrae": (
                    np.nanmean(track_results_mrae)
                    if len(track_results_mrae) > 0
                    else float("nan")
                ),
                "median_mrae": (
                    np.nanmedian(track_results_mrae)
                    if len(track_results_mrae) > 0
                    else float("nan")
                ),
                "std_mrae": (
                    np.std(track_results_mrae)
                    if len(track_results_mrae) > 0
                    else float("nan")
                ),
        ...
        track_re_fc.append(
            {
                ...
                "track_name": track_name_order[track_idx],
                "mean_re_fc": (
                    np.nanmean(track_results_re_fc)
                    if len(track_results_re_fc) > 0
                    else float("nan")
                ),
                "median_re_fc": (
                    np.nanmedian(track_results_re_fc)
                    if len(track_results_re_fc) > 0
                    else float("nan")
                ),
                "std_re_fc": (
                    np.nanstd(track_results_re_fc)
                    if len(track_results_re_fc) > 0
                    else float("nan")
                ),
...
def evaluate_model(
    ...
    true_values, pred_values, sampled_rows = _make_predictions(
        model, test_loader, device, model_resolution
    )
...
def load_model_for_testing(base_path: str, skip_data=False, device="cuda:0"):
    ...
    from yorzoi.train import Config
