...
from yorzoi.eval.brooks.dataset import BrooksDataset
    ...
    *meta* is a dict that at least contains: ``row_idx``, ``+_file``,
    ...
    if not pathlib.Path(brooks_eval_df_path).exists():
        raise FileNotFoundError(
            f"{brooks_eval_df_path} not found. Run create_eval_df() first."
        )
...
def evaluate(
    ...
    model_resolution=10,
    brooks_eval_df_path="brooks_eval_df.pkl",
    output_dir: str = "",
):
...
def merge_overlaps(
    ...
    for key, tree in trees_by_strain.items():
        ...
        for iv in intervals:
            ...
            if (
                start <= current_end
                and (max(current_end, end) - current_start) <= max_len
            ):
...
def add_bigwig_coverage(
    ...
    for _, row in df_intervals.iterrows():
        ...
        if strain_key is None:
            print(
                f"[warning] No bigWig entry for strain '{strain_raw}'. Skipping coverage."
            )
        ...
        try:
            ...
            print(
                f"[warning] Failed to extract + coverage for {strain_raw}:{chrom}:{start}-{end}: {e}"
            )
            vals_plus = None
...
def create_eval_df(brooks_sup_table: str = "science.abg0162_table_s3.txt"):
    import pandas as pd
    from yorzoi.eval.brooks.utils import (
        build_trees_by_strain,
        merge_overlaps,
        df_from_interval_trees,
        get_bigwig_dict,
        add_bigwig_coverage,
    )
    ...
    trees = build_trees_by_strain(
        df[df["noveljcn"] & (df["clusterEnd "] - df["clusterStart"] < 5000)]
    )
...
def _plot_all_predictions(
    df: "pd.DataFrame", strand: str = "+", ncols: int = 5, figsize_per_plot=(4, 2)
):
    ...
    valid_df = df[df[pred_col].notnull() & df[true_col].notnull()].reset_index(
        drop=True
    )
...
def create_train_df(
    ...
    if not plus_tracks or not minus_tracks:
        raise ValueError(
            "track_annotation.json must define non-empty '+' and '-' lists"
        )
    ...
    for idx, row in brooks_df.iterrows():
        ...
        try:
            plus_cov = _extract_centered_coverage(
                bw_plus, chrom, start, end, window=3000
            )
            minus_cov = _extract_centered_coverage(
                bw_minus, chrom, start, end, window=3000
            )
        except Exception as err:
            print(
                f"[warning] Coverage extraction failed for row {idx}: {err}. Skipping."
            )
            continue
